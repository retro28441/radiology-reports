...
st.title("🩻 CTV brain Report by Bright")
...
with col1:
    ...
    history = st.text_area("History", "Case of xx, presented with xx. Physical examination revealed GCS of E4V5M6 without focal neurological deficit. The CT scan was requested to evaluate cerebral venous sinus thrombosis.")
    techniques = st.text_area("Techniques", "Plain, CTV and contrast-enhanced axial helical CT scan of the brain was performed using 2.5-mm and 1.25-mm slice thicknesses with multiplanar reconstruction.")
    ...
    st.subheader("CTV")
    ctv_throm = smart_selectbox("Venous thrombosis", options["ctv_throm"])
    ctv_no = smart_selectbox("CTV No", options["ctv_no"])

    st.write("---")

    st.subheader("IMPRESSION")
    imp_ctv = smart_selectbox("CTV", options["imp_ctv"])
...
with col2:
    ...
    report_text = f"{title}VENOGRAPHY OF THE BRAIN\n\n"
    ...
    report_text += f"\nCTV\n"
    report_text += f"{ctv_throm} {ctv_no}\n"
