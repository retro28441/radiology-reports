...
st.title("🩻 CT brain +C Pressure Report by Bright")
...
with col1:
    ...
    history = st.text_area("History", "Case of xx, presented with xx. Physical examination revealed GCS of E4V5M6 without focal neurological deficit. The CT scan was requested to evaluation.")
    techniques = st.text_area("Techniques", "Axial helical scan of the brain was performed using 2.5-mm slice thickness with coronal and sagittal reconstructions.")
    ...
    sub_cerebral = smart_selectbox("Cerebral parenchyma", options["title_cerebralpres"], label_visibility="collapsed")
    cerebral_free = st.text_area("Free text", "The current study reveals $")

    num_ber1 = smart_selectbox("Infarction", options["num_ber2"])
    cerebral_gliotic = smart_selectbox("Hypodense", options["cerebral_gliotic2"], label_visibility="collapsed")
    cerebral_infarc = smart_selectbox("Infarct", options["cerebral_infarc2"], label_visibility="collapsed")
    num_ber2 = smart_selectbox("White matter changes", options["num_ber2"])
    wm_hypo =  smart_selectbox("WM hypodense", options["wm_hypo2"], label_visibility="collapsed")
    wm_loca = smart_selectbox("WM location", options["wm_loca2"], label_visibility="collapsed")
    wm_disease = smart_selectbox("WM Disease", options["wm_disease2"], label_visibility="collapsed")
    num_ber3 = smart_selectbox("Chronic lacunar infarct/Dilated perivascular space", options["num_ber2"])
    lacunar_perivas = smart_selectbox("well-defined hypodense", options["lacunar_perivas2"], label_visibility="collapsed")
    ...
    st.subheader("Associated pressure effect")
    ass_pres = smart_selectbox("Effacement", options["ass_pres"])
    mid_shift = smart_selectbox("Midline shift", options["mid_shift"])
    brain_herniat = smart_selectbox("Brain herniation", options["brain_herniat"])
...
with col2:
    ...
    report_text += f"{sub_cerebral}: \n- {cerebral_free}\n" 
    report_text += f"- {ass_pres} {mid_shift} {brain_herniat}\n"
    report_text += f"- {num_ber1} {cerebral_gliotic} {cerebral_infarc}\n"
    report_text += f"- {num_ber2} {wm_hypo} {wm_loca} {wm_disease}\n- {num_ber3} {lacunar_perivas}\n"
    report_text += f"- {cerebral_rest} {cerebral_no} {cerebral_contrast}\n"
    report_text += f"- {ven_tricle}\n"
    ...
    report_text += f"\nIMPRESSION:\n- {imp_free}\n- {ass_pres} {mid_shift} {brain_herniat}\n- {imp_braincon}\n- {imp_small}{imp_lacun}"
