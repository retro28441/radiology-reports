...
st.title("🩻 CTA CHEST Report by Bright")
...
with col1:
    ...
    history = st.text_area("History", "Case of xx, presented with hemoptysis for xx. The current CT was requested for evaluation.")
    techniques = st.text_area("Techniques", "Axial helical scan of the chest with 1.25 and 2.5-mm slice thickness with multiplanar reformations.")
    ...
    cta_chest = st.text_area("CTA chest", "No active contrast extravasation, aneurysm, dissection, or intramural hematoma. No bronchial artery dilatation.")
    ...
    pulmo_no = st.text_area("PA No", "No pulmonary embolism, pulmonary artery pseudoaneurysm or active contrast extravasation.")
    ...
    CTA_no = st.text_area("CTA NO", "No evidence of active contrast extravasation, aortic dissection, aneurysm or bronchial artery dilatation.")
...
with col2:
    ...
    report_text = f"{title}ANGIOGRAPHY OF THE CHEST\n\n"
    ...
    report_text += f"CTA\n"
    report_text += build_line(sub_aorta, aorta_side, vessels_chest, cta_chest)
    report_text += build_line(sub_pulmo, pulmo_artery, pulmo_no)
