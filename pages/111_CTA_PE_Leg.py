...
st.title("🩻 CTA for PE (+leg) Report by Bright")
...
with col1:
    ...
    techniques = st.text_area("Techniques", "Axial helical CTA scan of the pulmonary arteries (1-mm slice thickness) and CTV of the lower extremity veins (3-mm slice thickness).")
    ...
    st.subheader("Lower extremity CTV")
    sub_lowctv = st.text_input("Title", "Lower extremity veins", label_visibility="collapsed")
    lowctv_adeq = smart_selectbox("Adequacy of study2", options["adeq_both"], label_visibility="collapsed")
    low_ctv = st.text_input("Lower extremity veins", "No gross filling defect.")
    Inc_pelvis = smart_selectbox("Bladder", options["urinary_bladder"])
    pelvis_free = st.text_input("Free text", "", label_visibility="collapsed")
    ...
    CTA_no = st.text_input("CTA NO", "No evidence of acute pulmonary embolism or acute deep vein thrombosis.")
...
with col2:
    ...
    report_text = f"{title}PULMONARY ANGIOGRAPHY WITH MDCT VENOGRAPHY\n\n"
    ...
    report_text += f"\nLower extremity CTV\n"
    report_text += f"Adequacy of study: {lowctv_adeq}\n"
    report_text += build_line(sub_lowctv, low_ctv)
    report_text += f"Included pelvis: {Inc_pelvis} {pelvis_free}\n"
