...
st.title("🩻 CTA brain+neck stroke protocol Report by Bright")
...
with col1:
    ...
    history = st.text_area("History", "Case of xx, presented with xx. The physical examination shows E4V5M6, no facial palsy, motor grade V all extremities [NIHSS = x]. The study was requested to evaluate acute stroke.")
    techniques = st.text_area("Techniques", "Plain, multiphase CTA and contrast-enhanced CT scans of the brain and CTA of the neck were performed using 2.5-mm and 1.25-mm slice thicknesses with multiplanar reconstruction.")
    ...
    sub_aspectnc = st.text_input("Anterior circulation NC", "Alberta Stroke Program Early CT score (ASPECTS)", label_visibility="collapsed")
    aspect_nc = st.text_input("Anterior circulation NC score", "10/10", label_visibility="collapsed")
    sub_aspectcta = st.text_input("Anterior circulation CTA", "CTA-source image (CTA-SI) ASPECTS", label_visibility="collapsed")
    aspect_cta = st.text_input("Anterior circulation CTA score", "10/10", label_visibility="collapsed")
    sub_aspectpost = st.text_input("Posterior circulation", "Posterior-fossa ASPECTS", label_visibility="collapsed")
    aspect_post = st.text_input("Posterior circulation score", "10/10", label_visibility="collapsed")
    ...
    neck_frac = smart_selectbox("Fracture spine", options["neck_frac"], label_visibility="collapsed")
    ...
    st.subheader("Neck CTA")
    sub_cca = st.text_input("CCA", "Bilateral common carotid arteries (CCAs)")
    ctaneck_cca = smart_selectbox("CCA stenosis", options["ctaneck_cca"], label_visibility="collapsed")
    cca_no = smart_selectbox("CCA NO", options["cta_no"], label_visibility="collapsed")
    
    sub_bulb = st.text_input("Bulbs", "Bilateral carotid bulbs")
    ctaneck_bulb = smart_selectbox("Bulb stenosis", options["ctaneck_bulb"], label_visibility="collapsed")
    bulb_no = smart_selectbox("Bulb NO", options["cta_no"], label_visibility="collapsed")

    sub_cerica = st.text_input("Cervical ICA", "Bilateral internal carotid arteries (ICAs)")
    ctaneck_ica = smart_selectbox("Cervical ICA stenosis", options["ctaneck_ica"], label_visibility="collapsed")
    cerica_no = smart_selectbox("Cervical ICA NO", options["cta_no"], label_visibility="collapsed")

    sub_eca = st.text_input("ECA", "Bilateral external carotid arteries (ECAs)")
    ctaneck_eca = smart_selectbox("ECA stenosis", options["ctaneck_eca"], label_visibility="collapsed")
    eca_no = smart_selectbox("ECA NO", options["cta_no"], label_visibility="collapsed")

    sub_cerva = st.text_input("Cervical VA", "Bilateral vertebral arteries (VAs)")
    cerva_hypo = smart_selectbox("Cervical VA hypoplasia", options["va_hypo"], label_visibility="collapsed")
    ctaneck_va = smart_selectbox("Cervical VA stenosis", options["ctaneck_va"], label_visibility="collapsed")
    cerva_no = smart_selectbox("Cervical VA NO", options["cta_no"], label_visibility="collapsed")

    sub_aorta = st.text_input("Aorta", "Aortic arch and branches within thorax")
    cta_aorta = st.text_area("Aortic", "Left-sided aortic arch with normal branching pattern. No dissection or aneurysm.", label_visibility="collapsed")
...
with col2:
    ...
    report_text = f"{title}ANGIOGRAPHY OF THE BRAIN AND NECK (Stroke protocol)\n\n"
    ...
    report_text += build_line(sub_aspectnc, aspect_nc)
    report_text += build_line(sub_aspectcta, aspect_cta)
    report_text += build_line(sub_postfossa, cerebell_stem)
    report_text += build_line(sub_aspectpost, aspect_post)
    ...
    report_text += build_line(sub_bone, neck_bone, neck_frac, bone_lytic)
    ...
    report_text += f"\nNeck vasculature:\n"
    report_text += build_line(sub_cca, ctaneck_cca, cca_no)
    report_text += build_line(sub_bulb, ctaneck_bulb, bulb_no)
    report_text += build_line(sub_cerica, ctaneck_ica, cerica_no)
    report_text += build_line(sub_eca, ctaneck_eca, eca_no)
    report_text += build_line(sub_cerva, cerva_hypo, ctaneck_va, cerva_no)
    report_text += build_line(sub_aorta, cta_aorta)

    report_text += f"\n"
    report_text += build_line(sub_venous, venous_throm)
    report_text += f"*Criteria and image examples of collaterals are developed by the University of Calgary and can be viewed at www.aspectsinstroke.com/collateral-scoring/introduction/\n"
