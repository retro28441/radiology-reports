...
st.title("🩻 US KUB Report by Bright")
...
with col1:
    ...
    sub_ltkidney = st.text_input("Title", "Left kidney", label_visibility="collapsed")
    ltkidney_sizeparen = smart_selectbox("Left Status", options["onekidney_sizeparen"])
    ltkidney_cyst = smart_selectbox("Left Cyst", options["onekidney_cyst"])
    ltkidney_stone = smart_selectbox("Left Stone", options["onekidney_stone"])
    ltkidney_NO = smart_selectbox("Left kidney NO", options["us_kid_NO"])
...
with col2:
    ...
    report_text = f"{title} OF THE KUB SYSTEM\n\n"
    ...
    report_text += build_line(sub_ltkidney, ltkidney_sizeparen, ltkidney_cyst, ltkidney_stone, ltkidney_NO)
